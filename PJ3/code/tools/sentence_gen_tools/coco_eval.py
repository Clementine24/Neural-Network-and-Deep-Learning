...
def score_dcc(gt_template_novel,
  ...
  for word in words:
    ...
    for word2 in words:
      if word == word2:
        continue
      gt_json_train = read_json(gt_template_novel % (word2, dset))
      gt_ids_train_temp = [c['image_id'] for c in gt_json_train['annotations']]
      gt_ids_train.extend(gt_ids_train_temp)
    gen = []
