...
from .utils import strokes2drawing, to_normal_strokes, rec_incomplete_strokes, disc2stroke5, strokes2drawing_generate
...
class SketchTransformerTrainer(Trainer):
    ...
    def train(self):
        ...
        for epoch_i in range(self.args.num_epoch):
            ...
            for data_i, batch_data in tqdm(enumerate(self.datas['train_loader']), total=self.iter_per_epoch):
                end_time = time.time()
    ...
    def generate(self, mask_prob):
        ...
        if total_strokes_pred[0] is not None:
            ...
            self.update_generate_results(total_strokes, total_strokes_pred, total_input_masks, mask_prob)
    ...
    def update_stroke_results(self, mode, strokes, strokes_pred, masks, size=128):
        ...
        for i in range(min(len(strokes), self.args.num_display_samples)):
            ...
            if tmp_stroke_incomp[0][2] == 1:
                tmp_stroke_incomp[0][2] = 0
                tmp_stroke_incomp[0][3] = 1
    ...
    def update_generate_results(self, strokes, strokes_pred, masks, mask_prob):
        # true_strokes = strokes.clone()
        strokes_pred_type = strokes_pred[:,:,2:5]
        strokes_pred[:, :, 2:5] = strokes_pred_type == strokes_pred_type.max(dim=2, keepdim=True)[0]
        _tmp_stroke_pred = strokes_pred
        ...
        if self.mean is None and self.std is None:
            xy_s = []
            for sketch in strokes:
                for stroke in sketch:
                    if stroke[4] == 1:
                        break
                    xy_s.append(stroke[:2])
            xy_s = torch.stack(xy_s)
            self.mean = xy_s.mean(dim=0)
            self.std = xy_s.std(dim=0)
        norm_x = torch.normal(self.mean[0].item(), self.std[0].item(), size=(len(strokes), 300, 1))
        norm_y = torch.normal(self.mean[1].item(), self.std[1].item(), size=(len(strokes), 300, 1))
        # [for stroke in total_strokes]
        norm_xy = torch.cat((norm_x, norm_y), dim=2)
        comp_strokes_pred[1:, :, :2] += 0.1 * norm_xy[1:]
        ...
        for i in range(min(len(strokes), self.args.num_display_samples)):
            ...
            strokes2drawing_generate(tmp_comp_stroke_pred, size=50, svg_filename=f'{self.tmp_dir}/OrcBert_{round(mask_prob, 3)}_{i}.svg')
