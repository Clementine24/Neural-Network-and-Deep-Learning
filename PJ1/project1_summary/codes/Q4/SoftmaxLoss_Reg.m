function [f,g] = SoftmaxLoss_Reg(w,X,y,nHidden,nLabels,lambda)

[nInstances,nVars] = size(X);
% fprintf('X shape is %f,%f\n',size(X,1),size(X,2));

f = 0; % Count regular loss

% Form Weights
inputWeights = reshape(w(1:nVars*nHidden(1)),nVars,nHidden(1)); %因为输入的X已经是加过bias项的了，因此不需要再处理
f = f + lambda*sum(sum(inputWeights.^2,1),2);
...
for h = 2:length(nHidden)
  ...
  f = f + lambda*sum(sum(hiddenWeights{h-1}.^2,1),2);
end
...
f = f + lambda*sum(sum(hiddenWeights{end}.^2,1),2);
...
f = f + sum(-log(yhat(y==1)));  % 计算交叉熵损失，为了减小计算量，这里是挑出来再取对数

if nargout > 1
    err = yhat - y;
    for h = length(nHidden):-1:1
       gHidden{h} = fp{h}' * err + 2*lambda*hiddenWeights{h};  % 要记得加上正则化的梯度
