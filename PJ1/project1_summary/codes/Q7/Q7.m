...
Iteration = [];
w_pred = w;
% drop-out 概率，一般来说不在输入层做drop-out，因此我们将第一层设为0.0
drop_prob = [0.0,0.5];
tic;
% Use Softmax classification
funObj = @(w,i)SoftmaxLoss_Drop(w,X(i,:),yExpanded(i,:),nHidden,nLabels,drop_prob);
for iter = 1:maxIter
...
xlabel('Train iteration(step size=1e-3)');
ylabel('Error');
