...
for i = 1:nInstances   %这里使用的是一次只关注一个样本，因此后面每次W的梯度都要自加和，本来就是梯度相当于每个样本的梯度加上去
    ...
    fp{1} = tanh(ip{1});  %激活函数为tanh函数，后面可以改一下
    for h = 2:length(nHidden)
    ...
    if nargout > 1
        ...
        if length(nHidden) > 1
            ...
            for c = 1:nLabels
                ...
                backprop(c,:) = err(c)*(sech(ip{end}).^2.*outputWeights(:,c)'); %计算Z_L的梯度，注意这里是点积
            ...
            for h = length(nHidden)-2:-1:1
                backprop = (backprop*hiddenWeights{h+1}').*sech(ip{h+1}).^2; %注意这里W本来就比IP快一个单位，也是点乘
